...
class TimeStampImage {
  ...
  createImage() {
    ...
    image.onload = () => {
      ...
      if (
        this.data.site_url &&
        this.data.site_url === window.location.href &&
        this.data.timeStamp
      ) {
        playFromTimeStamp.addEventListener('click', () => {
          this.config.onPlayClick(this.data.timeStamp);
        });
